...
class TabuSearch(object):
    ...
    def __call__(self, neighborhood_function, knapsack):
        ...
        while self.iter_counter - self.iter_better < self.max_iter:
            self.iter_counter += 1
            if not len(sorted_moves) == 0:  # If the tabu list not eliminated all from neighbors
                candidate_move = sorted_moves.pop(0) # Get the best possible move
                ...
                if best_tabu.movement_avaliation > 0: # If it present a real improvement in the current solution
